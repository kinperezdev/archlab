...
- **In-app terminal.** Type real commands (`ls`, `pwd`, `git status`, ...). `cd` into any folder and ArchLab immediately maps that directory onto the canvas. Supports drag-and-drop file and folder uploads (or file-picker selection), automatically base64-uploading assets to a temporary working directory on the backend and returning the path for shell integration. Per-connection working directory, command history, localhost-bound.
