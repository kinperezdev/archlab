...
- **Database schema designer.** A SQL editor beside a live table-node canvas. Type or paste `CREATE TABLE` statements and the canvas generates table nodes with columns and types, drawing foreign-key relationships between them. Gated behind an active project to avoid layout confusion, it features confirmed vs inferred table grouping (color-coded containers), collapsed views (summarizing column counts to keep layouts clean while expanding to full list on selection), graphical PK/FK indicators (🔑/🔗), unresolved reference warnings, and styled relationship edge labels with tooltip diagnostics. Edits sync both ways, and tables can be pushed into the Ideas canvas to sketch a full database-to-backend-to-frontend system.
